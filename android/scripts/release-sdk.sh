...
echo "Releasing Exentriq Meet SDK ${SDK_VERSION}"
...
# Now build and publish the Exentriq Meet SDK and its dependencies
echo "Building and publishing the Exentriq Meet SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $DO_GIT_TAG == 1 ]]; then
    ...
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Exentriq Meet SDK + dependencies: ${SDK_VERSION}"
    popd
